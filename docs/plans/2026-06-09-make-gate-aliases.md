...
The repository had a working `make check` static Android baseline, but the
...
verifier on hosts without the legacy Android toolchain.

## Objectives

- Add `make lint`, `make test`, and `make build` aliases.
- Keep `make check` and `make verify` on the static Android baseline.
