...
class MakefileRootTests(unittest.TestCase):
    def test_absolute_makefile_path_with_spaces_and_apostrophe(self):
        with tempfile.TemporaryDirectory(prefix="OCR Scanner's gate ") as directory:
            checkout = Path(directory)
            makefile = checkout / "Makefile"
            makefile.write_text((ROOT / "Makefile").read_text(encoding="utf-8"), encoding="utf-8")
            result = subprocess.run(
                ["make", "-n", "-f", str(makefile), "verify"], cwd=checkout.parent,
            ...
            self.assertIn(str(checkout / "scripts" / "run-host-tests.sh"), result.stdout)

    def test_makefile_list_override_fails_closed(self):
        result = subprocess.run(
            ["make", "-n", "-f", str(ROOT / "Makefile"), "MAKEFILE_LIST=/tmp/untrusted", "verify"],
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, check=False,
            env={"PATH": os.environ.get("PATH", "")},
        )
